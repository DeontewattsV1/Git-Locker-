...
```mermaid
graph TD
    subgraph "Git Lockers Vault"
        A[Vault Lobby] --> B[Locker Wall]
        A --> C[Workbench]
        A --> D[Action Bay]
        A --> E[Audit Room]
    end

    subgraph "Locker Wall"
        B --> B1[Repo Idea Lockers]
        B --> B2[Prompt Capsule Lockers]
        B --> B3[GitHub Actions Lockers]
        B --> B4[JEAI Reasoning Lockers]
    end

    subgraph "Action Bay"
        D --> D1[validate-locker.yml]
        D --> D2[ci.yml]
        D --> D3[orchestrate-prompt-capsule.yml]
        D --> D4[build-prompt-docs.yml]
    end

    subgraph "Data Layer"
        F[(git-locker.csv)] --> G[validate-locker.py]
